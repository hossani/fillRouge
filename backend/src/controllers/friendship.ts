...
res.status(200).json({ status: friendship.status });
    ...
    if(error instanceof BadRequestError || error instanceof NotFoundError) return res.status(error.statusCode).
    json({ message: error.message });
 ...
 const deleteFriendship= async (req: AuthenticatedRequest, res: Response) => {
...
export { sendFriendRequest,acceptFriendRequest, rejectFriendRequest ,getFriends, getFriendsPending
  ,checkFriendStatus,deleteFriendship
};
