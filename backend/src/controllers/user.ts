...
import joiSchema from '../helpers/joiSchema';
...
const updateUserProfile = async (req: AuthenticatedRequest, res: Response) => {
  ...
  const { fullName, bio, birthDate, locationId, sportId,picture } = req.body;
  try {
    const { error } = joiSchema.userUpdateSchema.validate(
      { fullName, bio });
    ...
    if (age < 10 || age > 80) throw new BadRequestError('Age must be between 10 and 80 years old.');
      updatedData.birthDate = new Date(birthDate);
    }
