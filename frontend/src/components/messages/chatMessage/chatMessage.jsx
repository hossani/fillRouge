...
const ChatMessage = ({ player,handleChat }) => {
  ...
  const {userId, notificationMessage, setNotificationMessage,setNotificationOpenMessage}=useContext(AuthContext);
...
useEffect(()=>{
  if(notificationMessage.length==0) setNotificationOpenMessage(false);
},[notificationMessage])

// Scroll automatiquement vers le bas lorsque de nouveaux messages sont ajoutés
useEffect(() => {
...
setMessageInput("");
  ...
  return (
    <>
      <div id="chat-container" className={`fixed bottom-0 right-4 w-96 transition-all duration-300 ${isOpen ? 'h-auto' : ''} z-50`}>
